/**
 * The library's one visual for numbers, and its third runtime module (`runtime.ts`).
 *
 * The other two are wrappers — `SortableRows` and `Overlay` add behaviour to markup a
 * template already wrote, which is the rule `EmitModule` states. This one breaks it, and
 * the reason is worth writing down: a chart's markup *is* its data. Five rows are five
 * rects at coordinates nothing knows until the query answers, so there is no template to
 * wrap. The other named transforms (`options`, `tableRows`) expand a shape that is known
 * while generating; this one cannot be, because the shape arrives at run time.
 *
 * So D6 is kept the stronger way the repo already keeps `powerbi.ts` and `values.ts`:
 * one component written twice, pinned body-for-body by `runtime.test.ts`. The canvas and
 * the export do not merely render the same elements — they run the same code.
 */
